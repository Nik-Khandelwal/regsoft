...
class FirewallzTestCase(object):
	def setUp(self):
		self.user = User.objects.create_user(username='foo', password='foobar',
											 email='user@test.com', admin_level = 2)
		self.Firewallz_user = Firewallz_user.objects.create(user=self.user)
		self.Money = Money.objects.create(twothousand=10,fivehundred=10,twohundred=10,hundred=10,fifty=10)
		self.Team = Team.objects.create(college="Bits", city="Pilani", state="Rajasthan")
		...
		for i in range(0,1001):
			...
			Regpl = Regplayer.objects.create(name=us,gender="female",college=self.Team.college,city=self.Team.city)
			Grp = Group.objects.create(group_code=str(i), group_leader=1)
			Sp = Sport.objects.create(sport=str(i), gender="male")
			#Enteredpl = Enteredplayer.objects.create(regplayer=Regpl, group=Grp)
			#print(Enteredpl.pk)
			bil = Billcontrols.objects.create()


class MainTestCase(FirewallzTestCase, TestCase):
	def test_main(self):
	...
	def test_confirm_group(self):
		...
		response = self.client.post('/firewallz/confirm_group/',
								json.dumps(myObj),
								content_type="application/json")
		self.assertEqual(response.status_code, 200)
		response_json = json.loads(response.content)
		if not response_json.has_key('groupcode'):
			raise RuntimeError("confirm_group failed")
	...
	def test_unconfirm_details(self):
		self.client.login(username='foo', password='foobar')
		url = '/firewallz/unconfirm_details/'
		response = self.client.get(url)
		self.client.login(username='foo', password='foobar')
		self.assertEqual(response.status_code, 200)
		response_json = json.loads(response.content)
	...
	def test_sportlist(self):
		self.client.login(username='foo', password='foobar')
		url = '/firewallz/sportlist/'
		response = self.client.get(url)
		self.client.login(username='foo', password='foobar')
		self.assertEqual(response.status_code, 200)


class Main2TestCase(TestCase):
	def setUp(self):
		self.user = User.objects.create_user(username='foo', password='foobar',
											 email='user@test.com', admin_level = 2)
		self.Firewallz_user = Firewallz_user.objects.create(user=self.user)
	...
	def test_unconfirm_player(self):
		...
		response = self.client.post('/firewallz/unconfirm_player/',
								json.dumps(send_obj),
								content_type="application/json")
		self.assertEqual(response.status_code, 200)
		response_json = json.loads(response.content)

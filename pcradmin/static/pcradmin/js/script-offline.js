...
function fetchSwitchStatus() {
  ...
  ourRequest.onreadystatechange = function() {
    if (ourRequest.readyState === 4 && ourRequest.status === 200) {
      ...
      var jsonResponse = {"data": [["bits hyderabad", "h", "h", 26, []]], "data2": [["bits goa", "g", "g", 24, [[69, "grpleader", 9999999999, "f2016250@pilani.bits-pilani.ac.in", ["badminton girls", "badminton boys", "cricket"], 0, "male"]]]]};
      leftData = jsonResponse.data;
      rightData = jsonResponse.data2;
      for (var i = 0; i < leftData.length; i++) {
        document.getElementById('switch-status-left-list-clg').innerHTML += '<tr onclick="openCollegeLeaders(this, 1)" class="switch-status-college-row"> <td style="display: none">'+leftData[i][3]+'</td><td style="flex-basis: 40%">'+leftData[i][0]+'</td><td style="flex-basis: 30%">'+leftData[i][1]+'</td><td style="flex-basis: 30%">'+leftData[i][2]+'</td></tr>';
  ...
  ourRequest.onreadystatechange = function() {
    if (ourRequest.readyState === 4 && ourRequest.status === 200) {
      ...
      for (var i = 0; i < limits.length; i++) {
        document.getElementById('sport-limit-body').innerHTML += '<tr> <td style="display: none;">1</td><td style="flex-basis: 25%;">'+limits[i][0]+'</td><td style="flex-basis: 25%;">'+limits[i][1]+'</td><td style="flex-basis: 30%;">'+limits[i][2]+'</td><td style="flex-basis: 20%;" onclick="editSportLimit(this);"><i class="material-icons">build</i></td></tr>';
      }
      ...
      Materialize.toast('There was some error connecting to the server!', 3000);
      var limits = [[1, "Cricket (Boys)", 4, 7],[2, "Cricket (Girls)", 4, 7],[3, "Basketball (Boys)", 8, 11],[4, "Badminton (Boys)", 2, 3],[5, "Badminton (Girls)", 2, 4],[6, "Swimming (Girls)", 1, 1]];
      for (var i = 0; i < limits.length; i++) {
  ...
  ourRequest.onreadystatechange = function() {
    if (ourRequest.readyState === 4 && ourRequest.status === 200) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        document.getElementById('edit-part-sport-body').innerHTML += '<tr class="edit-part-sport-row"> <td style="flex-basis: 20%;">'+data[i][0]+'</td><td style="flex-basis: 25%;">'+sports+'</td><td style="flex-basis: 15%;">'+data[i][2]+'</td><td style="flex-basis: 30%;">'+data[i][3]+'</td><td style="flex-basis: 10%;" class="change_cursor" onclick="editSportPart(this)"><i class="material-icons">edit</i></td><td style="display: none;">'+data[i][4]+'</td></tr>';
      }
    }
  };
  ourRequest.send(sendData);
...
function fetchExcel() {
  ...
  ourRequest.onreadystatechange = function() {
    if (ourRequest.readyState === 4 && ourRequest.status === 200) {
      ...
      var leaders = jsonResponse.groupleaders;
      for (var i = 0; i < leaders.length; i++) {
        document.getElementById('download-excel-body').innerHTML += '<tr> <td style="flex-basis: 25%;">'+leaders[i][0]+'</td><td style="flex-basis: 30%;">'+leaders[i][1]+', '+leaders[i][2]+', '+leaders[i][3]+'</td><td style="flex-basis: 15%;"><a href="download_excel/'+leaders[i][4]+'/" target="_blank"><i class="material-icons black-text">file_download</i></a></td><td style="flex-basis: 15%;"><a href="download_csv/'+leaders[i][4]+'/" target="_blank"><i class="material-icons black-text">file_download</i></a></td><td style="flex-basis: 15%;"><a href="download_json/'+leaders[i][4]+'/" target="_blank"><i class="material-icons black-text">file_download</i></a></td><td style="display: none;" class="download-excel-groupleader-id">'+leaders[i][4]+'</td></tr>';
      }
      closeAllModals();
      showDiv(6);
    }
...
function confirmTeams() {
  ...
  var clg_id = parseInt(document.getElementsByClassName('unconfirmed-teams-status-selected')[i].children[5].innerHTML);
  var jsonData = {
    "id_arr": idnos,
    "clg_id": clg_id
  }
...
function unconfirmTeams() {
  ...
  var clg_id = parseInt(document.getElementsByClassName('confirmed-teams-status-selected')[i].children[5].innerHTML);
  var jsonData = {
    "id_arr": idnos,
    "clg_id": clg_id
  }
...
function openVerifyDocsGroupleader(option) {
  ...
  csrf_token = getCookie('csrftoken');
  var sendData = JSON.stringify(jsonData);
  document.getElementById('parts-view-docs-list-body').innerHTML = '';
  modal_open(14);
  var ourRequest = new XMLHttpRequest();
  var url = "url";
  ourRequest.open("POST", url, true);
  ...
  ourRequest.onreadystatechange = function() {
    if (ourRequest.readyState === 4 && ourRequest.status === 200) {
      ...
      for (var i = 0; i < participants.length; i++) {
        var sports = '';
        for (var j = 0; j < participants[i][2].length; j++) {
          sports += participants[i][2][j];
          if (j != participants[i][2].length - 1) {
            sports += ', ';
          }
        }
        document.getElementById('parts-view-docs-list-body').innerHTML += '<tr> <td style="display: none;">'+participants[i][0]+'</td><td style="flex-basis: 40%;">'+participants[i][1]+'</td><td style="flex-basis: 50%">'+sports+'</td><td style="flex-basis: 10%;"><a href=\"'+participants[i][3]+'\" target="_blank"><i class="material-icons black-text">file_download</i></a></td></tr>';
      }
    }
  };
  ourRequest.send(sendData);
...
function fetchDashboardData(num) {
  ...
  ourRequest.onreadystatechange = function() {
    if (ourRequest.readyState === 4 && ourRequest.status === 200) {
      ...
      var data = jsonResponse.data;
      if (num == 0) {
        document.getElementById('no-registered-body').innerHTML = '<tr> <td>'+data[0]+'</td><td>'+data[1]+'</td><td>'+data[2]+'</td></tr>';
      } else if (num == 1) {
        document.getElementById('no-confirmed-body').innerHTML = '<tr> <td>'+data[0]+'</td><td>'+data[1]+'</td><td>'+data[2]+'</td></tr>';
      } else if (num == 2) {
        document.getElementById('no-documents-body').innerHTML = '<tr> <td>'+data[0]+'</td><td>'+data[1]+'</td><td>'+data[2]+'</td></tr>';
      } else {
        document.getElementById('payment-body').innerHTML = '<tr> <td>'+data[0]+'</td><td>'+data[1]+'</td></tr>';
      }
      closeAllModals();
    } else if (ourRequest.readyState === 4 && ourRequest.status != 200) {
      Materialize.toast('There was some error connecting to the server!', 3000);

    ...
    ourRequest.onreadystatechange = function () {
      if (ourRequest.readyState === 4 && ourRequest.status === 200) {
        ...
        for (var i = 0; i < recieve_json.length; i++) {
            document.getElementById("uncnfrm_grp").innerHTML+="<div class='uncnfrm' ><span>"+recieve_json[i].name+"</span><i style='float: right;cursor: pointer' onclick='uncnfrm(this);'><img src='/static/firewallz/firewallz_unconfirm/icons/del.svg' class='unconfirm_icon'></i><span style='display: none;'>"+recieve_json[i].id+"</span></div>";
        }
        Materialize.toast('Updated List!', 3000);
      }
      else if (ourRequest.readyState === 4 && ourRequest.status != 200) {
        // document.getElementById("uncnfrm_grp").innerHTML="";
        // var recieve_json = [{"name": "Arpit Anshuman", "college": "BITS Pilani", "id": "12"},{"name": "Rampalli Sai Srivatsa", "college": "BITS Pilani", "id": "14"}]
        Materialize.toast('Error Fetching List!', 3000);
        // for (var i = 0; i < recieve_json.length; i++) {
        //     document.getElementById("uncnfrm_grp").innerHTML+="<div class='uncnfrm' ><span>"+recieve_json[i].name+"</span><i style='float: right;cursor: pointer' onclick='uncnfrm(this);'><img src='firewallz_unconfirm/icons/del.svg' class='unconfirm_icon'></i><span style='display: none;'>"+recieve_json[i].id+"</span></div>";
        // }
      }
    }
...
function uncnfrm(del_data) {
    ...
    var id = parseInt(del_data.parentElement.getElementsByTagName('span')[1].innerHTML);
    var csrf_token = getCookie('csrftoken');
    var ourRequest = new XMLHttpRequest();
...
function getGroups() {
    Materialize.toast('Fetching Group Leaders!', 4000);
    var csrf_token = getCookie('csrftoken');
    var ourRequest = new XMLHttpRequest();
    ...
    ourRequest.onreadystatechange = function () {
      if (ourRequest.readyState === 4 && ourRequest.status === 200) {
        document.getElementById("confirmed_grps_list").innerHTML="";
        var recieve_json = JSON.parse(ourRequest.responseText);
        console.log(recieve_json);
        for (var i = 0; i < recieve_json.length; i++) {
            document.getElementById("confirmed_grps_list").innerHTML+='<div class="cnfrmd_grps" onclick="open_details(this);"><span>'+recieve_json[i].pk+'</span>) <span>'+recieve_json[i].name+' </span>:<span> '+recieve_json[i].college+'</span><span class="idd">'+recieve_json[i].groupid+'</span> </div>';
        }
      }
      else if (ourRequest.readyState === 4 && ourRequest.status != 200) {
        Materialize.toast('Error Fetching List!', 3000);
        // document.getElementById("confirmed_grps_list").innerHTML="";
        // var recieve_json = [{"pk": 45, "name": "Arpit Anshuman", "college": "BITS Pilani", "groupid": "GHAJHA"},{"pk": 87, "name": "Rampalli Sai Srivatsa", "college": "BITS Pilani", "groupid": "GHAJHA"}]
        // for (var i = 0; i < recieve_json.length; i++) {
        //     document.getElementById("confirmed_grps_list").innerHTML+='<div class="cnfrmd_grps" onclick="open_details(this);"><span>'+recieve_json[i].pk+'</span>) <span>'+recieve_json[i].name+' </span>:<span> '+recieve_json[i].college+'</span><span class="idd">'+recieve_json[i].groupid+'</span> </div>';
        // }
      }
...
function search() {
  ...
  var grplist = document.getElementById('confirmed_grps_list');
  for (var i = 0; i < grplist.getElementsByTagName('div').length; i++) {

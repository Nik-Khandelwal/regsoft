...
function fetchBhawanStats() {
  ...
  var csrf_token = getCookie('csrftoken');
  var ourRequest = new XMLHttpRequest();
  ourRequest.open("GET", "https://api.myjson.com/bins/1d7569", true); // method and url
  ourRequest.setRequestHeader("Content-type", "application/json");
  // ourRequest.setRequestHeader("X-CSRFToken", csrf_token);
  ourRequest.onload = function () {
    if (ourRequest.status >= 200 && ourRequest.status < 400) { // request sent and recieved
...
function updateSelectBhawans(data, participants) {
  ...
  for(var i = 0; i < data.fields.length; i++) {
    if (data.fields[i].rooms != undefined) {
      ...
      document.getElementById("rooms-collection").innerHTML += '<li class="collection-item"> <ul class="collapsible" data-collapsible="expandable"> <li> <div class="collapsible-header"><i class="material-icons">hotel</i>'+data.fields[i].name+'</div><div class="collapsible-body custom-collapsible-body"></div></li>';
      collection_size++;
      var count=0;
      ...
      document.getElementById("rooms-collection").innerHTML += '<li class="collection-item avatar"> <i class="material-icons circle">hotel</i> <span class="title">'+data.fields[i].name+'</span> <p class="acco-avail">'+data.fields[i].no+'</p><a onclick="selectBhawan('+data.fields[i].id+')" class="secondary-content"><i class="material-icons">done</i></a> </li>';
    }
  }
  $(document).ready(function(){
    $('.collapsible').collapsible();

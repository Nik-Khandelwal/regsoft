...
function reg_top() {
  ...
  if(document.getElementById('top_add')) {
...
function team_addMoreSports() {
  ...
  if (team_formData.length > (2*num_extra_participants) + 4) {
  ...
  for (var i = 0; i < num_extra_participants; i++) {
    if (team_coach_gender=='') {
      extra_participants_details[2*i] = team_formData[4+(2*i)].value;
      extra_participants_details[(2*i)+1] = team_formData[5+(2*i)].value;
    } else {
      extra_participants_details[2*i] = team_formData[5+(2*i)].value;
      extra_participants_details[(2*i)+1] = team_formData[6+(2*i)].value;
    }
  }
  ...
  for (var i = 0; i < num_extra_participants; i++) {
    if (!validatePhoneNumber(extra_participants_details[(2*i)+1])) {
      check_extra_participants_phone = false;
      break;
    }
    if (extra_participants_details[2*i]=='' || extra_participants_details[(2*i)+1]=='') {
  ...
  if (team_coach_name != '' && team_coach_gender == '') {
    ...
    Materialize.toast('Must fill both Coach Fields!', 4000);
  } else if (!validateEmail(team_captain_email) || !validatePhoneNumber(team_captain_phone) || !check_extra_participants_phone) {
...
function getSportParticipants(sport_id, sport) {
  ...
  ajaxRequest.onreadystatechange = function() {
    if (ajaxRequest.readyState === 4 && ajaxRequest.status === 200) {
        ...
        for(var x in people) {
          if(sport_id!=people[x].fields.captain) {
            sport.parentElement.children[3].innerHTML+="<li class='people'>"+people[x].fields.name+"<span class='edit-name-icon badge black-text' onclick='editname("+sport_id+",\""+people[x].fields.name+"\")'><i class='material-icons'>edit</i></span></li>";
          }
          else {
...
function create_user_model(add_sports_arr) {
  ...
  for (var i = 0; i < add_sports_arr.length; i++) {
    sport_id_arr = '0000000000000000000000000000000000000000';
    captain = 0;
    if (i==0) {
    ...
    if (add_sports_arr[i] == "") {
      sport_id_arr = sport_id_arr.replaceAt((temp.sport_id-1), '1');
    } else {
      sport_id_arr = sport_id_arr.replaceAt((temp.sport_id-1), '1');
      sport_id_arr = sport_id_arr.replaceAt((add_sports_arr[i]-1), '1');
    }
    if (i==0) {
      ...
      backend_data["users"].push({
        ...
        "coach": 0,
        "sport_id": sport_id_arr,
        "name": temp.participants[(2*i) - 2],
        "email": "",
        "phone": temp.participants[(2*i) - 1],
        "gender": temp.gender
      });
    }
  }

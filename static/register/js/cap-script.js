function updateDisplayOptions() {
  var count = parseInt(document.getElementById('reg_no_count').innerHTML);
  if (count > 0) {
    document.getElementById('add_team_part_disp').style.display = 'inline-block';
  } else {
    document.getElementById('add_team_part_disp').style.display = 'none';
  }
}
function reg(sport_name, sport_id, coach, gender, lower, upper) {
...
function createUserModel(data, numPart) {
  ...
  sendUsers(backend_data, numPart);
}
...
function sendUsers(data, numPart) {
  console.log(data);
  var csrf_token = getCookie('csrftoken');
  ...
  ajaxRequest.onreadystatechange = function() {
    if (ajaxRequest.readyState === 4 && ajaxRequest.status === 200) {
      ...
      if (jsonResponse.error != null) {
        ...
        Materialize.toast('Your Partipants have been Saved!', 2000);
        document.getElementById('reg_no_count').innerHTML = parseInt(document.getElementById('reg_no_count').innerHTML) + numPart;
        updateDisplayOptions();
      }
    } else if (ajaxRequest.readyState === 4 && ajaxRequest.status != 200) {
      Materialize.toast('Error While Saving!', 2000);
  ...
  ajaxRequest.onreadystatechange = function() {
    if (ajaxRequest.readyState === 4 && ajaxRequest.status === 200) {
      ...
      if (jsonResponse.error != null) {
        ...
        Materialize.toast('Your Partipants have been Saved!', 2000);
        document.getElementById('reg_no_count').innerHTML = parseInt(document.getElementById('reg_no_count').innerHTML) + 1;
        updateDisplayOptions();
      }
    } else if (ajaxRequest.readyState === 4 && ajaxRequest.status != 200) {
      Materialize.toast('Error While Saving!', 2000);
